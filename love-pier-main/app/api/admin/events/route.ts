...
export async function POST(req: Request) {
  ...
  const [row] = await db.insert(events).values({
    ...
    registrationInfoTh: body.registrationInfoTh ?? '',
    registrationInfoEn: body.registrationInfoEn ?? '',
    registrationInfoZh: body.registrationInfoZh ?? '',
    ...
    albumImages: Array.isArray(body.albumImages) ? body.albumImages.slice(0, 10) : [],
