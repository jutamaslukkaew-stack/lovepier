...
const empty = { nameTh: '', descriptionTh: '', category: 'อาหารพรีออเดอร์', price: '', unit: 'ชุด', minQuantity: '1', leadDays: '3', pickupStart: '', pickupEnd: '', dailyQuota: '', coverImageUrl: '', media: [] as Media[], status: 'draft' }

function formOf(item: PreorderItem) {
  return { nameTh: item.nameTh, descriptionTh: item.descriptionTh, category: item.category, price: item.price == null ? '' : String(item.price), unit: item.unit, minQuantity: String(item.minQuantity), leadDays: String(item.leadDays), pickupStart: item.pickupStart || '', pickupEnd: item.pickupEnd || '', dailyQuota: item.dailyQuota == null ? '' : String(item.dailyQuota), coverImageUrl: item.coverImageUrl || '', media: Array.isArray(item.media) ? item.media as Media[] : [], status: item.status }
}
...
export function PreorderMenuManager({ items }: { items: PreorderItem[] }) {
  ...
  if (editing) return (
    <Card><CardContent className="space-y-5 pt-6">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        {/* Blank on both = no restriction, the dish inherits the shop's whole
            pre-order window. The END is the last slot bookable, not a closing
            time: 10:00–14:00 lets the customer pick 14:00. */}
        <div><Label>เริ่มรับได้</Label><Input type="time" value={form.pickupStart} onChange={(e) => set('pickupStart', e.target.value)} /><p className="mt-1 text-xs text-muted-foreground">เว้นว่างทั้งคู่ = รับได้ตามเวลาร้าน</p></div>
        <div><Label>รับได้ถึง (ช่องเวลาสุดท้าย)</Label><Input type="time" value={form.pickupEnd} onChange={(e) => set('pickupEnd', e.target.value)} /><p className="mt-1 text-xs text-muted-foreground">ตั้ง 14:00 = ลูกค้าเลือก 14:00 ได้</p></div>
  ...
  return <div className="space-y-4">
    ...
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{visible.map((item) => <Card key={item.id} className="overflow-hidden"><div className="aspect-video bg-gray-100">{item.coverImageUrl ? <img src={item.coverImageUrl} alt="" className="h-full w-full object-cover" /> : <div className="flex h-full items-center justify-center text-sm text-muted-foreground">ยังไม่มีรูป</div>}</div><CardContent className="space-y-3 pt-4"><div className="flex justify-between gap-3"><div><h3 className="font-semibold">{item.nameTh}</h3><p className="text-xs text-muted-foreground">{item.category} · ล่วงหน้า {item.leadDays} วัน{item.pickupStart && item.pickupEnd ? ` · รับ ${item.pickupStart}–${item.pickupEnd}` : ''}</p></div><span className="text-sm font-medium">{item.price == null ? 'ยังไม่ตั้งราคา' : `฿${item.price.toLocaleString()}`}</span></div><div className="flex flex-wrap gap-2">{item.isDeleted ? <Button size="sm" onClick={() => startTransition(async () => { await setPreorderItemDeleted(item.id, false); toast.success('กู้คืนแล้ว') })}><RotateCcw className="size-4" />กู้คืน</Button> : <><Button size="sm" variant="outline" onClick={() => open(item)}><Pencil className="size-4" />แก้ไข</Button><Button size="sm" variant={item.status === 'active' ? 'secondary' : 'outline'} onClick={() => startTransition(async () => { const next = item.status === 'active' ? 'paused' : 'active'; const result = await setPreorderItemStatus(item.id, next); if (!result.ok) toast.error(result.error); else toast.success(next === 'active' ? 'เปิดขายแล้ว' : 'ปิดรับแล้ว') })}>{item.status === 'active' ? 'ปิดรับ' : 'เปิดขาย'}</Button><Button size="sm" variant="destructive" onClick={() => { if (confirm(`ลบ “${item.nameTh}” ไปที่ถังขยะ?`)) startTransition(async () => { await setPreorderItemDeleted(item.id, true); toast.success('ย้ายไปถังขยะแล้ว') }) }}><Trash2 className="size-4" /></Button></>}</div></CardContent></Card>)}</div>
  </div>
}
