      ...
      <div className="border-t pt-5 space-y-4">
        ...
        <p className="text-xs text-muted-foreground">
          {(() => {
            ...
            return `ตอนนี้ลูกค้าจะเห็นช่องเวลา ${every} ตั้งแต่ ${w.startTime} ถึง ${w.endTime} (รับได้ถึงเวลาปิดร้าน — ถ้าไม่อยากส่งของตอนกำลังปิด ให้ตั้งช่วงเวลารับด้านบนให้จบก่อนเวลาปิด)`
          })()}
        </p>
        <div className="flex items-center justify-between gap-4 rounded-lg border px-3.5 py-3">
          <div className="space-y-0.5">
            ...
            <p className="text-xs text-muted-foreground">
              <strong>เปิดไว้เป็นค่าเริ่มต้น</strong> — ลูกค้าระบุเวลาที่ต้องการเองได้ เช่น 14:20 ไม่ต้องเลือกจากช่องเวลา
              แต่ยังต้องอยู่ในช่วงเวลารับด้านบน และตามเวลาสั่งล่วงหน้าขั้นต่ำเหมือนเดิม ·
              ปิดสวิตช์นี้ = ลูกค้าเลือกได้เฉพาะช่องเวลาที่ร้านกำหนด (ช่องหมายเหตุเรื่องเวลายังอยู่)
            </p>
          </div>
          <Switch
