...
ต้องตั้ง env (server-side):

| ตัวแปร | ได้จากไหน |
|--------|-----------|
| `GOOGLE_MAPS_API_KEY` | Google Cloud Console → เปิด **Routes API** + เปิด billing → สร้าง API key |
| `SHOP_LAT` / `SHOP_LNG` | พิกัดร้าน — เปิด Google Maps คลิกที่ร้าน แล้วก๊อปเลข lat, lng |
| `DELIVERY_RADIUS_KM` | รัศมีจัดส่ง (ค่าเริ่มต้น 5) |

> เว้น `GOOGLE_MAPS_API_KEY` ว่าง = ปิดเช็คระยะ (ปุ่มยังกดได้ แต่จะบอกว่าเช็คระยะไม่ได้ ให้ร้านเช็คเอง) — ระบบไม่ error
